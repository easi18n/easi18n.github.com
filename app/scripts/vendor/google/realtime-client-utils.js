...
rtclient.Authorizer = function() {
    this.clientId = rtclient.getOption('clientId');
...
rtclient.getFileMetadata = function(fileId, callback) {
    gapi.client.load('drive', 'v2', function() {
        gapi.client.drive.files.get({
            'fileId' : fileId || rtclient.params['fileId']
        }).execute(callback);
    });
}
...
rtclient.RealtimeLoader = function(options) {
  ...
  this.defaultTitle = rtclient.getOption('defaultTitle', 'New Realtime File');
 ...
 */
rtclient.RealtimeLoader.prototype.createNewFileAndRedirect = function(fileName) {
    //No fileId or state have been passed. We create a new Realtime file and
    // redirect to it.
    var _this = this;
    rtclient.createRealtimeFile(fileName || this.defaultTitle, function(file) {
        if (file.id) {
